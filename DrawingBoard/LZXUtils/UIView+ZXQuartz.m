...
//
- (void) drawTreeBarWithRect:(CGRect)rect Vals:(NSArray *)options refs:(NSArray *)refs num:(int)count
{
    ...
    [options enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
       ...
       // if (height <= 0.0f) height = T_HEIGHT;
        y = rect.size.height  - LBL_HEIGHT  - height ;
//        CGRect aframe = CGRectMake(20.0f + (rect.size.width/count ) * (idx % count) - (idx % count),y, rectWidth , height - T_HEIGHT);
        ...
        lblRef.text = [[options objectAtIndex:idx] valueForKey:@"text"];
        ...
        NSInteger ratio =  [[[options objectAtIndex:idx] valueForKey:@"value"] intValue];
        NSString *str = @"%";
        NSString *ratioStr = [[NSString stringWithFormat:@"%d",ratio] stringByAppendingFormat:@"%@",str];
        [ratioStr drawAtPoint:CGPointMake(aframe.origin.x, aframe.origin.y - 25 )
                     withFont:helveticaBold];
        
        /// Set color and draw the bar
//        if(iLen == maxHight)
//            iColor = [UIColor colorWithHexString:@"529f13"];//colorWithHexString:@"fc9b2e"
//        else
//            iColor = [UIColor lightGrayColor];//colorWithHexString:@"30a9d8"
//        
         /// Set color and draw the bar
        id rightbaby = [[options objectAtIndex:idx] valueForKey:@"text"];
...
//    pivotLabel.text = [NSString stringWithFormat:@"%.2f", maxLen];
    pivotLabel.text = @"统计";
