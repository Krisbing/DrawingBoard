...
//                if (([[[UIDevice currentDevice] systemVersion] floatValue] < 7.0f)&&(([[LZXAppDelegate sharedAppDelegate] isNowPPt]&&[[LZXAppDelegate sharedAppDelegate] isCourseNow])||[[LZXAppDelegate sharedAppDelegate] isProblemNow])) {
//                    return nil;
//                }
    ...
    @synchronized(self)
    {
        ...
        for (UIWindow *window  in [[UIApplication sharedApplication] windows])
        {
            if (![window respondsToSelector:@selector(screen)] || [window screen] == [UIScreen mainScreen])
            {
                ...
                // Code, such as a loop that creates a large number of temporary objects.
                // Render the layer hierarchy to the current context|
//                if (([[LZXAppDelegate sharedAppDelegate] isNowPPt]&&[[LZXAppDelegate sharedAppDelegate] isCourseNow])||[[LZXAppDelegate sharedAppDelegate] isProblemNow]) {}else
//                {
//                    return nil;
//                }
                [[window layer] renderInContext:context];//呈现接受者及其子范围到指定的上下文
